...
| --- | --- | --- |
| `lumibot/example_strategies/ai_trading_team_growth_inflation_equity_only_llm.py` | Added, 245 lines | Required conceptually, but port under the neutral mainline name `ai_trading_team_equity_only_llm.py`. Reuse helpers already present on `dev`. |
| `tests/test_ai_trading_team_growth_inflation_equity_only_llm.py` | Added, 251 lines | Required conceptually, but port under the neutral mainline test name `test_ai_trading_team_equity_only_llm.py`. |
| `scripts/compare_growth_inflation_quadrant_runs.py` | Added, 551 lines | Required conceptually, but port as neutral `compare_strategy_artifacts.py` rather than preserving quadrant-specific naming. |
| `tests/test_compare_growth_inflation_quadrant_runs.py` | Added, 263 lines | Required with the neutral comparison utility. Port metrics, multi-artifact comparison, and SPY buy-and-hold handling under neutral test names. |
| `scripts/run_ai_trading_team_examples_benchmark.py` | Modified, 10 lines | Required narrowly for the neutral `equity-only-llm` registry entry. Do not automatically port the archive branch's additional `growth-inflation-quadrant-fixed-etf-baseline` entry. Monthly frequency is allowed for this neutral mainline strategy. |
...
| Benchmark registry | Port narrowly | Add only the neutral `equity-only-llm` strategy key needed by this feature. Leave fixed ETF baseline out of this clean mainline branch. |
