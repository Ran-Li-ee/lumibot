...
def equity_basket_agent_system_prompt(symbols: str) -> str:
    return (
        ...
        "final holding count and target weights from candidate evidence plus local rank/volatility data. "
        "Use market_load_history_tables_summary first for multi-symbol comparison. "
...
def qqq_historical_equity_basket_agent_system_prompt(symbols: str) -> str:
    return (
        ...
        f"universe provided in basket_symbols ({symbols}). Return credible candidates supported by evidence. "
        ...
        "the final holding count and target weights from candidate evidence plus local rank/volatility data. Use "
        "market_load_history_tables_summary first for multi-symbol comparison. "
        f"{EQUITY_EVIDENCE_INTERPRETATION_POLICY} "
        "Do not invent sector, style, or category labels not provided by tools. "
...
def equity_basket_agent_task_prompt() -> str:
    return (
        ...
        "Build an ordered list of credible candidates from basket_symbols. selected_symbols must contain "
        "between 3 and 10 unique symbols from basket_symbols. Prefer fewer selected_symbols when leadership "
        "is narrow, and more when evidence is broad or close. Avoid selecting a stock supported by only one "
        "evidence group unless the other leading candidates are weaker or conflicting; explain the exception "
        "in reason_brief. "
